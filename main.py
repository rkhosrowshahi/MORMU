...
def main(args):
    ...
    unlearner_objs, obj_names, n_obj = None, [], 2
    if args.obj == "entropy_f1score":
        unlearner_objs = evaluate_entropy_f1score
        obj_names = ['Forget Entropy', 'Retain F1 score']
    elif args.obj == "3penalty_f1score":
        unlearner_objs = evaluate_3penalty_f1score
        obj_names = ['3-Penalty', 'Retain F1 score']
    else:
        raise ValueError('The unlearner objective type is not defined. Use the defined methods!')
    ...
    if os.path.exists(f"{checkpoint_dir}/unlearned_pareto_front_set_{args.steps}steps.npz"):
        ...
        unleaner_algorithm = mo_unlearner(dims=dims, n_obj=n_obj, n_steps=args.steps, 
                                        objectives=unlearner_objs, 
